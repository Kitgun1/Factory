...
namespace Factory
{
    public abstract class Structure : MonoBehaviour
    {
        ...
        protected bool Works = true;

        private bool _isWaited = false;
        ...
        private IEnumerator SendRoutine()
        {
            while (true)
            {
                if (Works)
                {
                    ...
                    List<StructurePoint> pointsInput = GetPointsByState(StructurePointState.Input);
                    ...
                    foreach (var pointInput in pointsInput)
                    {
                        ...
                        var structure = _mapManage.GetStructure(position.x + pointInput.Axis.x, position.y + pointInput.Axis.y);
                        if (structure != null)
                        {
                            var pointOutput = structure.GetPointByStateAndAxis(StructurePointState.Output, new Vector2Int(pointInput.Axis.x * -1, pointInput.Axis.y * -1));
                            if (pointOutput.Product != null && ProductIsPresentInStructure() == false)
                            {
                                pointInput.Product = pointOutput.Product;
                                pointInput.Product.transform.DOMove(StayPointProduct.position, Modifer[Level] / 2);
        ...
        public StructurePoint GetPointByStateAndAxis(StructurePointState state, Vector2Int axis)
        {
            var points = GetPointsByState(state);
        ...
        public StructurePoint GetPointByAxis(Vector2Int axis)
        {
        ...
        public List<StructurePoint> GetPointsByState(StructurePointState state)
        {
        ...
        public bool ProductIsPresentInStructure()
        {
            if (StructurePointsAxis.PointUp.Product != null)
                return true;
            if (StructurePointsAxis.PointRight.Product != null)
                return true;
            if (StructurePointsAxis.PointDown.Product != null)
                return true;
            if (StructurePointsAxis.PointLeft.Product != null)
                return true;
            return false;
        }
